    ...
    <div style={styles.shell}>
      ...
      {tab === "results" && (
        <ResultsTab
          ...
          onOpenCandidate={(file) => { setSelected(file); selectTab("gallery"); }}
        />
      )}
    ...
    <div style={styles.frontierLayout}>
      <section>
        ...
        <p style={styles.lead}>
          ...
          translated box window) strictly improves our reproducible baseline at{" "}
          <strong>{reproWinCount}</strong> of {sortedV2.length} probed n values
          (see table; ties at n=36 and n=49 do not count), but it does{" "}
  ...
  return (
    <div style={styles.frontierLayout}>
      ...
      <section>
        ...
        <ul style={{ ...styles.description, paddingLeft: 20 }}>
          <li>
            Close the n=64 Engel gap (251 vs 252): likely missing child moves
            or stronger canonization in the beam search.
          </li>
          <li>
            Promote Engel-Moser into the post-100 fallback baseline; the
            published-aware frontier still under-reports n=121..289.
          </li>
          <li>
            Improve Moser-ring search: more exact unit directions exist, but
            greedy windows do not yet exploit them.
          </li>
          <li>
            Run multi-worker CP-SAT on Linux (OR-Tools 9.15 hangs on Apple
            silicon); exact densest-k may still certify small induced seeds.
          </li>
          <li>
            Keep cut-and-project window work as a separate line: Z[ζ_12]
            translated boxes beat our reproducible baseline but not Engel
            et al. through n=100.
          </li>
        </ul>
        <p style={styles.description}>
          Repo docs: <code>README.md</code> (toolchain + tables),{" "}
          <code>NARRATIVE.md</code> (lessons learned). Regenerate shipped JSON
          and gallery plots with <code>make refresh-manifest</code> and the
          scripts under <code>scripts/</code>.
        </p>
      </section>
    </div>
  );
